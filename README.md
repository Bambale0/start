...
| **Коммуникации** | Яндекс Почта, Mail.ru, generic email, телефония, web, MAX, Telegram, VK |
...
# Срочное должно приходить само

Start не должен быть системой, которую нужно постоянно проверять вручную.

Новая аварийная заявка, срочное письмо, официальный документ, SLA-риск или критичный Incident должны **сами появляться у нужного человека**.

Для этого закладывается отдельный Attention / Notification Engine:
...
кому нужно знать
↓
мгновенное in-app уведомление
↓
push / messenger / email fallback
↓
ACK
↓
если реакции нет — эскалация
~~~

Поддерживаемые почтовые контуры включают **Яндекс Почту, Mail.ru и generic IMAP/SMTP**.

Получение почты проектируется near-real-time: push/webhook где доступно, IMAP watch где поддерживается, fallback polling + обязательная reconciliation-проверка.

При этом система борется с шумом: 100 сообщений об одной аварии должны превращаться в один Incident и одно управляемое срочное оповещение, а не в 100 всплывающих окон.

---

# Два AI-контура

Start проектируется не с одним всесильным ботом, а с двумя безопасными агентными режимами.

### Агент сотрудника

Работает по прямой команде человека:
...
По умолчанию он **читает**, а не меняет бизнес-состояние. Любое изменение проходит через обычные права, validation и Action Gateway.

### Агент автоматизации

Работает в фоне по заранее настроенным workflow:

- классифицирует входящее обращение;
- извлекает адрес/заявителя/проблему;
- предлагает маршрут;
- ищет вероятный дубль/Incident;
- классифицирует документ;
- готовит уведомление;
- формирует текстовую сводку.

Ему не нужен зоопарк из десятков инструментов. Он получает небольшой read-first набор возможностей и возвращает структурированное решение, а реальные изменения выполняет детерминированный backend.

Это позволяет автоматизировать рутину, не превращая LLM в привилегированный backend.
